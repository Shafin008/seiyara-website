...
$(document).ready(function() {
    ...
    // MODIFIED: Smooth scroll with better timing
    $('a[href^="#"]').click(function(e) {
        const target = $(this.getAttribute('href'));
        if (target.length) {
            e.preventDefault();
            ...
            $('html, body').stop(true, true).animate({
                scrollTop: target.offset().top - headerHeight - 20
            }, 600, 'easeInOutQuad'); // CHANGED: Balanced timing - not too fast, not too slow
        }
    });
    ...
    // ============================================
    // MODIFIED: REFINED SMOOTH SCROLL REVEAL ANIMATIONS
    // ============================================
    ...
    // MODIFIED: Refined reveal animation on scroll
    function revealOnScroll() {
        const windowHeight = $(window).height();
        const scrollTop = $(window).scrollTop();
        ...
        $('.reveal').each(function() {
            const elementTop = $(this).offset().top;
            const revealPoint = 120; // CHANGED: Balanced trigger point
    ...
    // Add stagger delays to solution cards
    $('.solution-card').each(function(index) {
        $(this).css('transition-delay', (index * 0.15) + 's'); // CHANGED: Back to 0.15s
    });
